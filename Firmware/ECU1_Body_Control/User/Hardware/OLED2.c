...
/**
  ...
  */
uint8_t OLED2_DisplayBuf[8][128];
...
/**
  ...
  */
void OLED2_W_SCL(uint8_t BitValue)
{
...
/**
  ...
  */
void OLED2_W_SDA(uint8_t BitValue)
{
...
{
	OLED2_W_SDA(1);		//释放SDA，确保SDA为高电平
	OLED2_W_SCL(1);		//释放SCL，确保SCL为高电平
	OLED2_W_SDA(0);		//在SCL高电平期间，拉低SDA，产生起始信号
	OLED2_W_SCL(0);		//起始后把SCL也拉低，即为了占用总线，也为了方便总线时序的拼接
}
...
{
	OLED2_W_SDA(0);		//拉低SDA，确保SDA为低电平
	OLED2_W_SCL(1);		//释放SCL，使SCL呈现高电平
	OLED2_W_SDA(1);		//在SCL高电平期间，释放SDA，产生终止信号
}
...
void OLED2_I2C_SendByte(uint8_t Byte)
{
	...
	for (i = 0; i < 8; i++)
	{
		...
		OLED2_W_SCL(1);	//释放SCL，从机在SCL高电平期间读取SDA
		OLED2_W_SCL(0);	//拉低SCL，主机开始发送下一位数据
	}
...
{
	OLED2_I2C_Start();				//I2C起始
	OLED2_I2C_SendByte(0x78);		//发送OLED的I2C从机地址
	OLED2_I2C_SendByte(0x00);		//控制字节，给0x00，表示即将写命令
	OLED2_I2C_SendByte(Command);		//写入指定的命令
	OLED2_I2C_Stop();				//I2C终止
}
...
/**
  ...
  * 返 回 值：无
  */
void OLED2_WriteData(uint8_t *Data, uint8_t Count)
{
	uint8_t i;
	
	OLED2_I2C_Start();				//I2C起始
	OLED2_I2C_SendByte(0x78);		//发送OLED的I2C从机地址
	OLED2_I2C_SendByte(0x40);		//控制字节，给0x40，表示即将写数据
	/*循环Count次，进行连续的数据写入*/
	for (i = 0; i < Count; i ++)
	{
		OLED2_I2C_SendByte(Data[i]);	//依次发送Data的每一个数据
	}
	OLED2_I2C_Stop();				//I2C终止
}
...
{
	OLED2_GPIO_Init();			//先调用底层的端口初始化
	
	/*写入一系列的命令，对OLED进行初始化配置*/
	OLED2_WriteCommand(0xAE);	//设置显示开启/关闭，0xAE关闭，0xAF开启
	
	OLED2_WriteCommand(0xD5);	//设置显示时钟分频比/振荡器频率
	OLED2_WriteCommand(0x80);	//0x00~0xFF
	
	OLED2_WriteCommand(0xA8);	//设置多路复用率
	OLED2_WriteCommand(0x3F);	//0x0E~0x3F
	
	OLED2_WriteCommand(0xD3);	//设置显示偏移
	OLED2_WriteCommand(0x00);	//0x00~0x7F
	
	OLED2_WriteCommand(0x40);	//设置显示开始行，0x40~0x7F
	...
	OLED2_WriteCommand(0x81);	//设置对比度
	OLED2_WriteCommand(0xCF);	//0x00~0xFF
	...
	OLED2_WriteCommand(0xA6);	//设置正常/反色显示，0xA6正常，0xA7反色
	...
	OLED2_Clear();				//清空显存数组
	OLED2_Update();				//更新显示，清屏，防止初始化后未显示内容时花屏
}
...
/**
  ...
  */
void OLED2_SetCursor(uint8_t Page, uint8_t X)
{
...
//	X += 2;
	...
	/*通过指令设置页地址和列地址*/
	OLED2_WriteCommand(0xB0 | Page);					//设置页位置
...
/**
  ...
  */
uint8_t OLED2_pnpoly(uint8_t nvert, int16_t *vertx, int16_t *verty, int16_t testx, int16_t testy)
{
	int16_t i, j, c = 0;
...
/**
  ...
  */
uint8_t OLED2_IsInAngle(int16_t X, int16_t Y, int16_t StartAngle, int16_t EndAngle)
{
	int16_t PointAngle;
...
void OLED2_Update(void)
{
	...
	for (j = 0; j < 8; j ++)
	{
		...
		OLED2_WriteData(OLED2_DisplayBuf[j], 128);
	}
}
...
void OLED2_UpdateArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Page; j < Page1; j ++)
	{
		if (X >= 0 && X <= 127 && j >= 0 && j <= 7)		//超出屏幕的内容不显示
		{
			...
			OLED2_WriteData(&OLED2_DisplayBuf[j][X], Width);
		}
	}
}
...
void OLED2_Clear(void)
{
	...
	for (j = 0; j < 8; j ++)				//遍历8页
	{
		...
		{
			OLED2_DisplayBuf[j][i] = 0x00;	//将显存数组数据全部清零
		}
	}
}
...
void OLED2_ClearArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Y; j < Y + Height; j ++)		//遍历指定页
	{
		for (i = X; i < X + Width; i ++)	//遍历指定列
		{
			...
			{
				OLED2_DisplayBuf[j / 8][i] &= ~(0x01 << (j % 8));	//将显存数组指定数据清零
			}
		}
	}
}
...
void OLED2_Reverse(void)
{
	...
	for (j = 0; j < 8; j ++)				//遍历8页
	{
		for (i = 0; i < 128; i ++)			//遍历128列
		{
			OLED2_DisplayBuf[j][i] ^= 0xFF;	//将显存数组数据全部取反
		}
	}
}
...
void OLED2_ReverseArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Y; j < Y + Height; j ++)		//遍历指定页
	{
		for (i = X; i < X + Width; i ++)	//遍历指定列
		{
			if (i >= 0 && i <= 127 && j >=0 && j <= 63)			//超出屏幕的内容不显示
			{
				OLED2_DisplayBuf[j / 8][i] ^= 0x01 << (j % 8);	//将显存数组指定数据取反
			}
		}
	}
}
...
void OLED2_ShowChar(int16_t X, int16_t Y, char Char, uint8_t FontSize)
{
	if (FontSize == OLED_8X16)		//字体为宽8像素，高16像素
	{
		...
		OLED2_ShowImage(X, Y, 8, 16, OLED_F8x16[Char - ' ']);
	}
	else if(FontSize == OLED_6X8)	//字体为宽6像素，高8像素
	{
		/*将ASCII字模库OLED_F6x8的指定数据以6*8的图像格式显示*/
		OLED2_ShowImage(X, Y, 6, 8, OLED_F6x8[Char - ' ']);
	}
}
...
void OLED2_ShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
	...
	while (String[i] != '\0')	//遍历字符串
	{
		...
		if (CharLength == 1)	//如果是单字节字符
		{
			...
			OLED2_ShowChar(X + XOffset, Y, SingleChar[0], FontSize);
			XOffset += FontSize;
		}
		else					//否则，即多字节字符
		{
			...
			if (FontSize == OLED_8X16)		//给定字体为8*16点阵
			{
				...
				OLED2_ShowImage(X + XOffset, Y, 16, 16, OLED_CF16x16[pIndex].Data);
				XOffset += 16;
			}
			else if (FontSize == OLED_6X8)	//给定字体为6*8点阵
			{
				/*空间不足，此位置显示'?'*/
				OLED2_ShowChar(X + XOffset, Y, '?', OLED_6X8);
				XOffset += OLED_6X8;
			}
		}
	}
...
void OLED2_ShowNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	for (i = 0; i < Length; i++)		//遍历数字的每一位							
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		OLED2_ShowChar(X + i * FontSize, Y, Number / OLED2_Pow(10, Length - i - 1) % 10 + '0', FontSize);
	}
}
...
void OLED2_ShowSignedNum(int16_t X, int16_t Y, int32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	if (Number >= 0)						//数字大于等于0
	{
		OLED2_ShowChar(X, Y, '+', FontSize);	//显示+号
	...
	for (i = 0; i < Length; i++)			//遍历数字的每一位								
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		OLED2_ShowChar(X + (i + 1) * FontSize, Y, Number1 / OLED2_Pow(10, Length - i - 1) % 10 + '0', FontSize);
	}
}
...
void OLED2_ShowHexNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	for (i = 0; i < Length; i++)		//遍历数字的每一位
	{
		...
		if (SingleNumber < 10)			//单个数字小于10
		{
			...
			/*+ '0' 可将数字转换为字符格式*/
			OLED2_ShowChar(X + i * FontSize, Y, SingleNumber + '0', FontSize);
		}
		else							//单个数字大于10
		{
			/*调用OLED_ShowChar函数，显示此数字*/
			/*+ 'A' 可将数字转换为从A开始的十六进制字符*/
			OLED2_ShowChar(X + i * FontSize, Y, SingleNumber - 10 + 'A', FontSize);
		}
	}
}
...
void OLED2_ShowBinNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	for (i = 0; i < Length; i++)		//遍历数字的每一位	
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		OLED2_ShowChar(X + i * FontSize, Y, Number / OLED2_Pow(2, Length - i - 1) % 2 + '0', FontSize);
	}
}
...
void OLED2_ShowFloatNum(int16_t X, int16_t Y, double Number, uint8_t IntLength, uint8_t FraLength, uint8_t FontSize)
{
	...
	PowNum = OLED2_Pow(10, FraLength);		//根据指定小数的位数，确定乘数
	FraNum = round(Number * PowNum);		//将小数乘到整数，同时四舍五入，避免显示误差
	IntNum += FraNum / PowNum;				//若四舍五入造成了进位，则需要再加给整数
	
	/*显示整数部分*/
	OLED2_ShowNum(X + FontSize, Y, IntNum, IntLength, FontSize);
	
	/*显示小数点*/
	OLED2_ShowChar(X + (IntLength + 1) * FontSize, Y, '.', FontSize);
	
	/*显示小数部分*/
	OLED2_ShowNum(X + (IntLength + 2) * FontSize, Y, FraNum, FraLength, FontSize);
}
...
void OLED2_ShowImage(int16_t X, int16_t Y, uint8_t Width, uint8_t Height, const uint8_t *Image)
{
	...
	/*将图像所在区域清空*/
	OLED2_ClearArea(X, Y, Width, Height);
	...
	for (j = 0; j < (Height - 1) / 8 + 1; j ++)
	{
		...
		for (i = 0; i < Width; i ++)
		{
			if (X + i >= 0 && X + i <= 127)		//超出屏幕的内容不显示
			{
				...
				{
					/*显示图像在当前页的内容*/
					OLED2_DisplayBuf[Page + j][X + i] |= Image[j * Width + i] << (Shift);
				}
				
				if (Page + j + 1 >= 0 && Page + j + 1 <= 7)		//超出屏幕的内容不显示
				{					
					/*显示图像在下一页的内容*/
					OLED2_DisplayBuf[Page + j + 1][X + i] |= Image[j * Width + i] >> (8 - Shift);
				}
			}
		}
	}
...
void OLED2_Printf(int16_t X, int16_t Y, uint8_t FontSize, char *format, ...)
{
	...
	va_end(arg);							//结束变量arg
	OLED2_ShowString(X, Y, String, FontSize);//OLED显示字符数组（字符串）
}
...
void OLED2_DrawPoint(int16_t X, int16_t Y)
{
	...
	{
		/*将显存数组指定位置的一个Bit数据置1*/
		OLED2_DisplayBuf[Y / 8][X] |= 0x01 << (Y % 8);
	}
}
...
uint8_t OLED2_GetPoint(int16_t X, int16_t Y)
{
	...
	{
		/*判断指定位置的数据*/
		if (OLED2_DisplayBuf[Y / 8][X] & 0x01 << (Y % 8))
		{
			return 1;	//为1，返回1
		}
	}
...
void OLED2_DrawLine(int16_t X0, int16_t Y0, int16_t X1, int16_t Y1)
{
	...
	else				//斜线
	{
		...
		/*画起始点，同时判断标志位，将坐标换回来*/
		if (yflag && xyflag){OLED2_DrawPoint(y, -x);}
		else if (yflag)		{OLED2_DrawPoint(x, -y);}
		else if (xyflag)	{OLED2_DrawPoint(y, x);}
		else				{OLED2_DrawPoint(x, y);}
...
void OLED2_DrawRectangle(int16_t X, int16_t Y, uint8_t Width, uint8_t Height, uint8_t IsFilled)
{
	...
	if (!IsFilled)		//指定矩形不填充
	{
		...
		{
			OLED2_DrawPoint(i, Y);
			OLED2_DrawPoint(i, Y + Height - 1);
		}
		/*遍历左右Y坐标，画矩形左右两条线*/
		for (i = Y; i < Y + Height; i ++)
		{
			OLED2_DrawPoint(X, i);
			OLED2_DrawPoint(X + Width - 1, i);
		}
	}
	else				//指定矩形填充
	{
...
void OLED2_DrawTriangle(int16_t X0, int16_t Y0, int16_t X1, int16_t Y1, int16_t X2, int16_t Y2, uint8_t IsFilled)
{
	...
	{
		/*调用画线函数，将三个点用直线连接*/
		OLED2_DrawLine(X0, Y0, X1, Y1);
		OLED2_DrawLine(X0, Y0, X2, Y2);
		OLED2_DrawLine(X1, Y1, X2, Y2);
	}
	else					//指定三角形填充
	{
		/*找到三个点最小的X、Y坐标*/
		...
		for (i = minx; i <= maxx; i ++)
		{
			...
			for (j = miny; j <= maxy; j ++)
			{
				...
				/*如果在，则画点，如果不在，则不做处理*/
				if (OLED2_pnpoly(3, vx, vy, i, j)) {OLED2_DrawPoint(i, j);}
			}
		}
	}
}
...
void OLED2_DrawCircle(int16_t X, int16_t Y, uint8_t Radius, uint8_t IsFilled)
{
	...
	/*画每个八分之一圆弧的起始点*/
	OLED2_DrawPoint(X + x, Y + y);
	OLED2_DrawPoint(X - x, Y - y);
	OLED2_DrawPoint(X + y, Y + x);
	OLED2_DrawPoint(X - y, Y - x);
...
void OLED2_DrawArc(int16_t X, int16_t Y, uint8_t Radius, int16_t StartAngle, int16_t EndAngle, uint8_t IsFilled)
{
	...
	if (OLED2_IsInAngle(x, y, StartAngle, EndAngle))	{OLED2_DrawPoint(X + x, Y + y);}
	if (OLED2_IsInAngle(-x, -y, StartAngle, EndAngle)) {OLED2_DrawPoint(X - x, Y - y);}
	if (OLED2_IsInAngle(y, x, StartAngle, EndAngle)) {OLED2_DrawPoint(X + y, Y + x);}
	if (OLED2_IsInAngle(-y, -x, StartAngle, EndAngle)) {OLED2_DrawPoint(X - y, Y - x);}
	...
	if (IsFilled)	//指定圆弧填充
	{
		...
		for (j = -y; j < y; j ++)
		{
			...
			if (OLED2_IsInAngle(0, j, StartAngle, EndAngle)) {OLED2_DrawPoint(X, Y + j);}
		}
	}
	...
	while (x < y)		//遍历X轴的每个点
	{
		...
		if (IsFilled)	//指定圆弧填充
		{
			...
			{
				/*在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理*/
				if (OLED2_IsInAngle(x, j, StartAngle, EndAngle)) {OLED2_DrawPoint(X + x, Y + j);}
				if (OLED2_IsInAngle(-x, j, StartAngle, EndAngle)) {OLED2_DrawPoint(X - x, Y + j);}
			}
			...
			{
				/*在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理*/
				if (OLED2_IsInAngle(-y, j, StartAngle, EndAngle)) {OLED2_DrawPoint(X - y, Y + j);}
				if (OLED2_IsInAngle(y, j, StartAngle, EndAngle)) {OLED2_DrawPoint(X + y, Y + j);}
			}
		}
	}
